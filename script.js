const backendUrl = "https://projectsubmission-wpmx.onrender.com/api"; // change this after deploy
...
$('login-btn').addEventListener('click', async () => {
  ...
  try{
    ...
    $('login-error').textContent = 'Invalid username or password. Check console for details.';
  }
});
...
async function loadTeacherProjects(){
  try{
    ...
    tbody.innerHTML = '';
    if(rows.length === 0){
      tbody.innerHTML = '<tr><td colspan="3">No projects yet. Create one!</td></tr>';
    } else {
      rows.forEach(p => {
        const tr = document.createElement('tr');
...
$('add-project-btn').addEventListener('click', ()=>{
  ...
  `, async ()=>{
    ...
    if(!title){
      ...
      throw new Error('Title is required'); // Throw to prevent modal from closing
    }
    try{
